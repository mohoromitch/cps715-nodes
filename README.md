...
In order to run the node, it must be run in the following format:

```
java -jar node.jar [protocol] [destination ip] [uri]
```
...
```
java -jar node.jar http 10.0.0.1 node1.net/status
```

This will run a node that sends GET requests to the IP of the proxy requesting the URI node1.net/status.
